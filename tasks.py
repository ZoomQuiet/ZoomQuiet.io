...
def build(c):
    ...
    # c.run('pelican {input_path} -v -o {deploy_path} -s pelicanconf.py'.format(**env))


def serve(c):
    # c.run('cd {deploy_path} && python -m SimpleHTTPServer'.format(**env))
    c.run("cd {deploy_path} && python3 -m http.server".format(**env))
...
def pull_data(c):
    c.run("cd {deploy_path} && " "pwd && " "git pull".format(**env))
